...
class DQN:
    ...
    def get_loss(self, batches):
        ...
        q_next = self.target_net.forward(next_states).max(dim=-1)[0].detach().reshape(-1, 1)

        target = (rewards + GAMMA * q_next)
        main = self.main_net.forward(states).gather(1, actions).reshape(-1, 1)
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/DQN_181103_GRAYSCALE')
...
def rgb2dataset(rgb_data):
    ...
    rgb_data = cv2.cvtColor(rgb_data, cv2.COLOR_BGR2GRAY)
    # Raw Image : (240, 256, 3)
